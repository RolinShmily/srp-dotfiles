...
个人 Debian 13 服务器环境配置文件仓库。通过符号链接（Symlink）将仓库内的配置文件部署至系统的主目录及 `~/.config` 目录。
...
下表列出各配置模块涉及的核心软件包在 Debian (`apt`) 中的包名映射：
...
| **代码编辑器** | Neovim & 工具链 | `git`, `build-essential`, `gcc`, `unzip` (Neovim v0.11.7 为二进制手动安装) | - |
...
本仓库的终端配置（包含 Spaceship 提示符符号、Neovim 等）依赖 Nerd Fonts 字体编码。

* **推荐字体**：[subframe7536/maple-font](https://github.com/subframe7536/maple-font) (**`Maple Mono NF CN`**)
* **字体安装**：
  * **Debian / Ubuntu**：从 [maple-font Releases](https://github.com/subframe7536/maple-font/releases) 下载 `.ttf` 文件，复制至 `~/.local/share/fonts/` 后执行 `fc-cache -fv`。
---
