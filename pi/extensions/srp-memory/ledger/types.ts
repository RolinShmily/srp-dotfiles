/**
 * Ledger custom-type vocabulary for observational memory.
 */
...
export function isCostData(value: unknown): value is CostEntryData {
  if (!isPlainRecord(value)) return false;
  return (
    typeof value.costUsd === "number" &&
    Number.isFinite(value.costUsd) &&
    value.costUsd >= 0 &&
    (value.role === "observer" || value.role === "consolidator") &&
    isNonEmptyString(value.runId)
  );
}

export function isObservationsRecordedEntry(
  entry: Entry,
): entry is Entry & { type: "custom"; customType: typeof OM_OBSERVATIONS_RECORDED; data: ObservationsRecordedEntryData } {
  return (
    entry.type === "custom" &&
...
export function isObservationsDroppedEntry(
  entry: Entry,
): entry is Entry & { type: "custom"; customType: typeof OM_OBSERVATIONS_DROPPED; data: ObservationsDroppedEntryData } {
  return (
    entry.type === "custom" &&
...
export function isCostEntry(
  entry: Entry,
): entry is Entry & { type: "custom"; customType: typeof OM_COST; data: CostEntryData } {
  if (entry.type !== "custom" || entry.customType !== OM_COST) return false;
  const data = entry.data as Record<string, unknown> | undefined;
  return (
    !!data &&
    typeof data.costUsd === "number" &&
    Number.isFinite(data.costUsd as number) &&
 ...
 * (`getEntries()`), NOT a single branch (`getBranch()`).
 */
