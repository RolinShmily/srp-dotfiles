...
/**
 * The orchestrator re-derives a precise, unique id-timestamp for each model observation
 * at commit. The model only emits minute resolution; the orchestrator turns that into a
 * "YYYY-MM-DDTHH:MM:SS[.NN]" id, appending a deterministic ".01", ".02", … disambiguator
 * when two observations resolve to the same second (within the batch or against the existing buffer).
 * `tokenCount` is computed here, never by the model.
 */
