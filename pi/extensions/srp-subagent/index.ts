...
async function launchSubagent(
  ...
  const surface = options?.surface ?? createSurface(effectiveName);
  ...
  if (agentDefs?.cli === "claude") {
    ...
    sendLongCommand(surface, command, {
      ...
      scriptPreamble: [
        `# Claude Code subagent launch script for ${effectiveName}`,
...
export default function subagentsExtension(pi: ExtensionAPI) {
  ...
  // /srp-subagent command — spawn a subagent by name
  const handleSubagentCommand = async (args: string, ctx: ExtensionContext) => {
    const trimmed = args.trim();
    if (!trimmed) {
      ctx.ui.notify("Usage: /srp-subagent <agent> [task]", "warning");
      return;
    }
  ...
  pi.registerCommand("srp-subagent", {
    description: "Spawn a subagent: /srp-subagent <agent> <task>",
